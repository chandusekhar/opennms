...
It is now possible to configure the flow listener to receive Netflow v5, Netflow v9, IPFIX and SFlow on a single port.
An example for configuring it is in the link:https://github.com/OpenNMS/opennms/blob/2840d3b3ced40b5b78120c635df6d3fb50fca91e/opennms-base-assembly/src/main/filtered/etc/telemetryd-configuration.xml#L95[$OPENNMS_HOME/etc/telemetryd-configuration.xml] file.
